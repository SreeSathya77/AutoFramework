...
def dump():
    with sync_playwright() as p:
        ...
        print("ACTIVITIES GRID:")
        page.locator('div[role="tab"]', has_text="Activities").click()
        page.wait_for_timeout(2000)
        
        rows = page.locator("table.sec-table tbody tr").all()
        for i, row in enumerate(rows):
            cells = row.locator("td").all_inner_texts()
            print(f"Row {i}: {cells}")
            
        print("CURRENT OWNER:", page.locator('select#caseOwner').input_value())
        
        print("CHANGE STATUS DROPDOWN:")
        status_dropdown = page.locator('select#caseStatus')
        if status_dropdown.is_visible():
            opts = status_dropdown.locator('option').all()
            for opt in opts:
                print(f"Status Option: {opt.inner_text().strip()}")
        else:
            print("Status dropdown not visible")
